            ...
            ([level, choice]) => {
              ...
              choices.forEach((singleChoice) => {
                ...
                if (typeof singleChoice === "string") {
                  ...
                  if (singleChoice.mainChoice) {
                    choiceName = singleChoice.mainChoice;
                    subChoiceName = singleChoice.subChoice;
                  } else {
                    choiceName =
                      singleChoice.name ||
                      singleChoice.selectedChoice ||
                      singleChoice.choice ||
                      String(singleChoice);
                  }
                }
                    ...
                    benefits.specialAbilities.forEach((ability) => {
                      ...
                      if (ability.damage) {
                        ...
                        if (typeof dmg === "object") {
                          let dmgStr = dmg.base || "";
                          if (dmg.scaling) {
                            const scalingParts = Object.entries(dmg.scaling)
                              .map(([lvl, val]) => `${val} at level ${lvl}`)
                              .join(", ");
                            dmgStr += ` (${scalingParts})`;
                          }
                          abilityDetails.push(`Damage: ${dmgStr}`);
                        } else {
                          abilityDetails.push(`Damage: ${dmg}`);
                        }
                      }
                      if (ability.range)
      ...
      if (character.heritageChoices?.[heritageName]) {
        Object.entries(character.heritageChoices[heritageName]).forEach(
          ([featureName, choiceName]) => {
            ...
            const choices = Array.isArray(choiceName)
              ? choiceName
              : [choiceName];
            ...
            choices.forEach((singleChoice) => {
              ...
              const selectedOption = feature?.options?.find(
                (opt) => opt.name === singleChoice
              );

              sections.innateHeritage.features.push({
                name: `${featureName}: ${singleChoice} (Selected)`,
                type: "Selected Heritage Choice",
                source: heritageName,
                level: null,
                description:
                  selectedOption?.description ||
                  `Selected ${singleChoice} for ${featureName}`,
                details: [
